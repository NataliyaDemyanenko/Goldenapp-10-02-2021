...
def goldenapp():
     ...
     dist = pdist(np.array(list(partyposition))[:, None],metric='euclidean')
     ...
     for k in range(0, party_num):
         for j in range(0, party_num):   
             if dist[j] > thr and k!=j+1:
                 #print(k,j,dist[j])
                 excluded[k] += parties[j+1]
                 print(k, excluded[k])

     #Exclude mutually
     for k in range(0, party_num):
         for j in range(0, party_num):   
             if dist[k] > thr and k+1 != j:
                 #print(k,j,dist[j])
                 excluded[k+1] += parties[j]
                 print(k+1, excluded[k+1])
